################################################################################
# Angela Oill
# Analysis: T-test comparing ACR vs AMR
################################################################################
...
## New groupings from heart atlas - 1% cells ----
...
clustered_obj <- readRDS("/scratch/aoill/projects/heart_transplant/heart_spatial_obj_denoist_2025_06_26.rds")
...
# sample numbers
clustered_obj_pre_AMR_CR@meta.data$patient_id
clustered_obj_pre_AMR_CR@meta.data$patient_rejection_type

tmp <- clustered_obj_pre_AMR_CR@meta.data %>% dplyr::select(patient_id, patient_rejection_type) %>% unique()
table(tmp$patient_rejection_type)
...
for (ctn in 1:length(cts_to_analyze)) {
  ...
  # Summarize by patient_id to calculate the mean expression for each gene and sample
  ...
  # add sample info
  sample_info <- obj_CT@meta.data %>% dplyr::select(patient_id, patient_rejection_type) %>% unique()
  mean_expression_per_sample_info <- left_join(mean_expression_per_sample, sample_info)
  
  
  if (min(table(mean_expression_per_sample_info$patient_rejection_type)) < 3 || (length(table(mean_expression_per_sample_info$patient_rejection_type)) < 2) ) {
    print(paste("Not enough samples or groups to do test. Cell type: ", ct, sep = ""))
    
    print(table(mean_expression_per_sample_info$patient_rejection_type))
    ...
    for (genei in genes_to_analyze) {
      ...
      genei_mean_expression_per_sample <- mean_expression_per_sample_info %>% dplyr::select(patient_id, patient_rejection_type, genei)
      colnames(genei_mean_expression_per_sample) <- c("patient_id", "patient_rejection_type", "expression")
      ...
      if (sum(genei_mean_expression_per_sample$expression > 0) >= 3) {
        ...
        ttest_result_ct_gene <- t.test(
          genei_mean_expression_per_sample%>% filter(patient_rejection_type == "cellular_rejection") %>% select("expression"),
          genei_mean_expression_per_sample%>% filter(patient_rejection_type == "antibody_rejection") %>% select("expression"),
        )
        ...
        # 1. Calculate mean expression for 'cellular_rejection'
        mean_cellular_rejection <- mean(genei_mean_expression_per_sample$expression[genei_mean_expression_per_sample$patient_rejection_type == "cellular_rejection"])
        
        # 2. Calculate mean expression for 'antibody_rejection'
        mean_antibody_rejection <- mean(genei_mean_expression_per_sample$expression[genei_mean_expression_per_sample$patient_rejection_type == "antibody_rejection"])
        
        # 3. Add pseudocount to the means
        mean_cellular_rejection_adjusted <- mean_cellular_rejection + pseudocount
        mean_antibody_rejection_adjusted <- mean_antibody_rejection + pseudocount
        
        # 4. Calculate the log2 fold change
        # Formula: log2(Mean Expression in cellular_rejection / Mean Expression in antibody_rejection)
        # A positive value means higher expression in cellular_rejection.
        # A negative value means higher expression in antibody_rejection.
        log2_fold_change <- log2(mean_cellular_rejection_adjusted / mean_antibody_rejection_adjusted)
...
# 70 sig genes across 11 cell types, 78 gene cell type pairs
...
write_csv(out_table, "/home/aoill/projects/heart_transplant/00_final/revisions/t_test_AMR_vs_CR_results.csv")
