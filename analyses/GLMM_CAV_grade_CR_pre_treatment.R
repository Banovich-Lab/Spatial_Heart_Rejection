...
saveRDS(cr_pre_cav_grade_all_CT_results, 
        "/scratch/aoill/projects/heart_transplant/00_final/GLMM_CAV_grade_CR_pre_treatment_biopsies_revisions.rds")
saveRDS(all_CT_results_with_issues, 
        "/scratch/aoill/projects/heart_transplant/00_final/GLMM_CAV_grade_CR_pre_treatment_biopsies_revisions_with_issues.rds")

cr_pre_cav_grade_all_CT_results <- readRDS("/scratch/aoill/projects/heart_transplant/00_final/GLMM_CAV_grade_CR_pre_treatment_biopsies_revisions.rds")

for (i in names(cr_pre_cav_grade_all_CT_results)) {
  print(i)
  cr_pre_cav_grade_all_CT_results[[i]]$p_value <- as.numeric(cr_pre_cav_grade_all_CT_results[[i]]$p_value)
  cr_pre_cav_grade_all_CT_results[[i]] <- cr_pre_cav_grade_all_CT_results[[i]] %>%
    drop_na(p_value)
  cr_pre_cav_grade_all_CT_results[[i]]$padj <- p.adjust(cr_pre_cav_grade_all_CT_results[[i]]$p_value, method = "fdr")
}

saveRDS(cr_pre_cav_grade_all_CT_results, 
        "/scratch/aoill/projects/heart_transplant/00_final/GLMM_CAV_grade_CR_pre_treatment_biopsies_revisions_fixed.rds")
...
cr_pre_cav_grade_all_CT_results_merged <- do.call(rbind, cr_pre_cav_grade_all_CT_results)
unique(cr_pre_cav_grade_all_CT_results_merged$ct)
table(cr_pre_cav_grade_all_CT_results_merged$ct)

# Keep only significant genes
cr_pre_cav_grade_all_CT_results_merged_sig <- cr_pre_cav_grade_all_CT_results_merged %>%
  filter(padj <= 0.05)
cr_pre_cav_grade_all_CT_results_merged_sig$n_samples_no0 <- as.numeric(cr_pre_cav_grade_all_CT_results_merged_sig$n_samples_no0
)
nrow(cr_pre_cav_grade_all_CT_results_merged_sig)
length(unique(cr_pre_cav_grade_all_CT_results_merged_sig$genei))
length(unique(cr_pre_cav_grade_all_CT_results_merged_sig$ct))
...
write.csv(cr_pre_cav_grade_all_CT_results_merged, "/home/aoill/projects/heart_transplant/00_final/revisions/GLMM_CAV_grade_CR_pre_treatment_biopsies_revisions_fixed.csv",
          row.names = F, quote = F)

write.csv(all_CT_results_with_issues_merged, "/home/aoill/projects/heart_transplant/00_final/revisions/GLMM_CAV_grade_CR_pre_treatment_biopsies_revisions_with_issues.csv",
          row.names = F, quote = F)


# Make a table with which gene cell type pairs had convergenc issues or small 
# variance issues
no_iss_results <- cr_pre_cav_grade_all_CT_results_merged
...
glmms_with_issues_tbl$test <- "CR pre-treatment across CAV"
...
write.csv(glmms_with_issues_tbl, 
          "/home/aoill/projects/heart_transplant/00_final/revisions/GLMM_CAV_grade_CR_pre_treatment_biopsies_convergence_issues.csv",
          row.names = F, quote = F)
