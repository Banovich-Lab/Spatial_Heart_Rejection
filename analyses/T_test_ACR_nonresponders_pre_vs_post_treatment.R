################################################################################
# Angela Oill
# Analysis: T-test comparing ACR non-responders pre- vs post-treatment 
################################################################################
...
# CR Patients that had persistence pre vs post treatment
...
# sample numbers
clustered_obj_pre_post_persistence@meta.data$patient_id
clustered_obj_pre_post_persistence@meta.data$biopsy_timing

tmp <- clustered_obj_pre_post_persistence@meta.data %>% dplyr::select(patient_id, biopsy_timing) %>% unique()
...
# 0 sig genes across 0 cell types, 0 gene cell type pairs
...
write_csv(out_table, "/home/aoill/projects/heart_transplant/00_final/revisions/t_test_ACR_nonresponders_pre_vs_post_treatment_results.csv")
