...
saveRDS(cr_post_cav_grade_all_CT_results, 
        "/scratch/aoill/projects/heart_transplant/00_final/GLMM_CAV_grade_CR_post_treatment_biopsies_revisions.rds")
saveRDS(all_CT_results_with_issues, 
        "/scratch/aoill/projects/heart_transplant/00_final/GLMM_CAV_grade_CR_post_treatment_biopsies_revisions_with_issues.rds")


cr_post_cav_grade_all_CT_results <- readRDS("/scratch/aoill/projects/heart_transplant/00_final/GLMM_CAV_grade_CR_post_treatment_biopsies_revisions.rds")

for (i in names(cr_post_cav_grade_all_CT_results)) {
  print(i)
  cr_post_cav_grade_all_CT_results[[i]]$p_value <- as.numeric(cr_post_cav_grade_all_CT_results[[i]]$p_value)
  cr_post_cav_grade_all_CT_results[[i]] <- cr_post_cav_grade_all_CT_results[[i]] %>%
    drop_na(p_value)
  cr_post_cav_grade_all_CT_results[[i]]$padj <- p.adjust(cr_post_cav_grade_all_CT_results[[i]]$p_value, method = "fdr")
}

saveRDS(cr_post_cav_grade_all_CT_results, 
        "/scratch/aoill/projects/heart_transplant/00_final/GLMM_CAV_grade_CR_post_treatment_biopsies_revisions_fixed.rds")
...
cr_post_cav_grade_all_CT_results_merged <- do.call(rbind, cr_post_cav_grade_all_CT_results)
unique(cr_post_cav_grade_all_CT_results_merged$ct)
table(cr_post_cav_grade_all_CT_results_merged$ct)

# Keep only significant genes
cr_post_cav_grade_all_CT_results_merged_sig <- cr_post_cav_grade_all_CT_results_merged %>%
  filter(padj <= 0.05)
cr_post_cav_grade_all_CT_results_merged_sig$n_samples_no0 <- as.numeric(cr_post_cav_grade_all_CT_results_merged_sig$n_samples_no0
)
nrow(cr_post_cav_grade_all_CT_results_merged_sig)
length(unique(cr_post_cav_grade_all_CT_results_merged_sig$genei))
length(unique(cr_post_cav_grade_all_CT_results_merged_sig$ct))
#   padj 0.05 - 45 significant genes across 16 cell types, 53 gene cell type pairs



# save merged as a csv 
# THIS IS SUPPLEMENTARY TABLE 8
write.csv(cr_post_cav_grade_all_CT_results_merged, "/home/aoill/projects/heart_transplant/00_final/revisions/GLMM_CAV_grade_CR_post_treatment_biopsies_revisions_fixed.csv",
          row.names = F, quote = F)

write.csv(all_CT_results_with_issues_merged, "/home/aoill/projects/heart_transplant/00_final/revisions/GLMM_CAV_grade_CR_post_treatment_biopsies_revisions_with_issues.csv",
          row.names = F, quote = F)


# Make a table with which gene cell type pairs had convergenc issues or small 
# variance issues
no_iss_results <- cr_post_cav_grade_all_CT_results_merged
...
glmms_with_issues_tbl$test <- "CR post-treatment across CAV"
...
write.csv(glmms_with_issues_tbl, 
          "/home/aoill/projects/heart_transplant/00_final/revisions/GLMM_CAV_grade_CR_post_treatment_biopsies_convergence_issues.csv",
          row.names = F, quote = F)
