...
# CR post-treatment - CAV score ----
## Subset CR post-treatment samples ----
post_CR_smpls <-  clustered_obj@meta.data %>% 
...
clustered_obj_linear_regression <- subset(clustered_obj, 
                                          subset = Sample %in% post_CR_smpls)
...
for (ctn in 1:length(cts_to_analyze)) {
  ...
  # Step: get average expostssion for each biopsy (Sample) in the seurat object 
  # for the cell type
  obj_CT <- subset(clustered_obj_linear_regression, subset = ct_second_pass == ct)
  
  # Extract the normalized expostssion data (data is log-transformed counts)
  ...
  # Summarize by Sample to calculate the mean expostssion for each gene and sample
  mean_expostssion_per_sample <- expr_matrix_t %>%
    group_by(Sample) %>%
    summarise(across(everything(), mean, na.rm = TRUE))
  
  # Convert back to a data frame 
  mean_expostssion_per_sample <- as.data.frame(mean_expostssion_per_sample)
  ...
  for (gene in 1:length(all_genes)) {
    ...
    # select gene
    genei_expostssion_counts <- mean_expostssion_per_sample %>%
      select(Sample, genei)
    ...
    genei_expostssion_counts_info <- left_join(genei_expostssion_counts, outcome_data)
    colnames(genei_expostssion_counts_info) <- c("Sample", "expostssion", "patient_id", 
                                                "cav_grade", "age_at_biopsy")
    genei_df <- genei_expostssion_counts_info
    
    
    # Fit the full model using tryCatch
    tryCatch({
      # Fit the model
      LM_result <- glmmTMB(expostssion ~ cav_grade + age_at_biopsy + (1 | patient_id), 
      ...
      genei_df_no_0 <- genei_df %>% filter(expostssion > 0)
  ...
  # New - expostssion in at least 5 samples
  ...
  #        "/scratch/aoill/projects/heart_transplant/GLMM_results_CAV_CR_post_treatment_all_rm_model_errors_fix_exp_20241126_in_progress_results.rds")
...
cr_post_cav_grade_all_CT_results <- all_CT_results

# Filter out empty lists
cr_post_cav_grade_all_CT_results_filter <- cr_post_cav_grade_all_CT_results[sapply(cr_post_cav_grade_all_CT_results, length) != 0]
cr_post_cav_grade_all_CT_results_filter <- cr_post_cav_grade_all_CT_results_filter[sapply(cr_post_cav_grade_all_CT_results_filter, nrow) != 0]

# SAVE # 
#saveRDS(cr_post_cav_grade_all_CT_results_filter, 
#        "/scratch/aoill/projects/heart_transplant/GLMM_results_CAV_CR_post_treatment_all_rm_model_errors_fix_exp_20241126_FIX.rds")


# Merge the list of data frames into one data frame
cr_post_cav_grade_all_CT_results_filter_merged <- do.call(rbind, cr_post_cav_grade_all_CT_results_filter)
unique(cr_post_cav_grade_all_CT_results_filter_merged$ct)
table(cr_post_cav_grade_all_CT_results_filter_merged$ct)



# Keep only significant genes
cr_post_cav_grade_all_CT_results_filter_merged_sig <- cr_post_cav_grade_all_CT_results_filter_merged %>%
  filter(padj <= 0.05)
cr_post_cav_grade_all_CT_results_filter_merged_sig$n_samples_no0 <- as.numeric(cr_post_cav_grade_all_CT_results_filter_merged_sig$n_samples_no0
)
nrow(cr_post_cav_grade_all_CT_results_filter_merged_sig)
length(unique(cr_post_cav_grade_all_CT_results_filter_merged_sig$genei))
length(unique(cr_post_cav_grade_all_CT_results_filter_merged_sig$ct))



# save merged as a csv 
# THIS IS SUPPLEMENTART TABLE 9
write.csv(cr_post_cav_grade_all_CT_results_filter_merged, "/home/aoill/projects/heart_transplant/00_final/GLMM_CAV_grade_CR_post_treatment_biopsies_NEW.csv",
          row.names = F, quote = F)