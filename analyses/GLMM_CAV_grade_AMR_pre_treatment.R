...
# AMR pre-treatment - CAV score ----
## Subset AMR pre-treatment samples ----
...
amr_pre_cav_grade_all_CT_results <- all_CT_results

saveRDS(amr_pre_cav_grade_all_CT_results, 
        "/scratch/aoill/projects/heart_transplant/00_final/GLMM_CAV_grade_AMR_pre_treatment_biopsies_revisions.rds")
saveRDS(all_CT_results_with_issues, 
        "/scratch/aoill/projects/heart_transplant/00_final/GLMM_CAV_grade_AMR_pre_treatment_biopsies_revisions_with_issues.rds")

all_CT_results_with_issues_merged <- do.call(rbind, all_CT_results_with_issues)

amr_pre_cav_grade_all_CT_results <- readRDS("/scratch/aoill/projects/heart_transplant/00_final/GLMM_CAV_grade_AMR_pre_treatment_biopsies_revisions.rds")

for (i in names(amr_pre_cav_grade_all_CT_results)) {
  print(i)
  amr_pre_cav_grade_all_CT_results[[i]]$p_value <- as.numeric(amr_pre_cav_grade_all_CT_results[[i]]$p_value)
  amr_pre_cav_grade_all_CT_results[[i]] <- amr_pre_cav_grade_all_CT_results[[i]] %>%
    drop_na(p_value)
  amr_pre_cav_grade_all_CT_results[[i]]$padj <- p.adjust(amr_pre_cav_grade_all_CT_results[[i]]$p_value, method = "fdr")
}

saveRDS(amr_pre_cav_grade_all_CT_results, 
        "/scratch/aoill/projects/heart_transplant/00_final/GLMM_CAV_grade_AMR_pre_treatment_biopsies_revisions_fixed.rds")



# Merge the list of data frames into one data frame
amr_pre_cav_grade_all_CT_results_merged <- do.call(rbind, amr_pre_cav_grade_all_CT_results)
unique(amr_pre_cav_grade_all_CT_results_merged$ct)
table(amr_pre_cav_grade_all_CT_results_merged$ct)

# Keep only significant genes
amr_pre_cav_grade_all_CT_results_merged_sig <- amr_pre_cav_grade_all_CT_results_merged %>%
  filter(padj <= 0.05)
amr_pre_cav_grade_all_CT_results_merged_sig$n_samples_no0 <- as.numeric(amr_pre_cav_grade_all_CT_results_merged_sig$n_samples_no0
)
nrow(amr_pre_cav_grade_all_CT_results_merged_sig)
length(unique(amr_pre_cav_grade_all_CT_results_merged_sig$genei))
length(unique(amr_pre_cav_grade_all_CT_results_merged_sig$ct))
#   padj 0.05 - 56 significant genes across 16 cell types, 62 gene cell type pairs



# save merged as a csv 
# THIS IS SUPPLEMENTARY TABLE 8
write.csv(amr_pre_cav_grade_all_CT_results_merged, "/home/aoill/projects/heart_transplant/00_final/revisions/GLMM_CAV_grade_AMR_pre_treatment_biopsies_revisions_fixed.csv",
          row.names = F, quote = F)

write.csv(all_CT_results_with_issues_merged, "/home/aoill/projects/heart_transplant/00_final/revisions/GLMM_CAV_grade_AMR_pre_treatment_biopsies_revisions_with_issues.csv",
          row.names = F, quote = F)

amr_pre_cav_grade_all_CT_results_merged <- read.csv("/home/aoill/projects/heart_transplant/00_final/revisions/GLMM_CAV_grade_AMR_pre_treatment_biopsies_revisions_fixed.csv")

# Make a table with which gene cell type pairs had convergenc issues or small 
# variance issues
no_iss_results <- amr_pre_cav_grade_all_CT_results_merged
...
glmms_with_issues_tbl$test <- "AMR pre-treatment across CAV"
...
write.csv(glmms_with_issues_tbl, 
          "/home/aoill/projects/heart_transplant/00_final/revisions/GLMM_CAV_grade_AMR_pre_treatment_biopsies_convergence_issues.csv",
          row.names = F, quote = F)
...
colnames(genei_expression_counts_info) <- c("Sample", "expression", "patient_id", 
                                            "cav_grade", "age_at_biopsy")
genei_df <- genei_expression_counts_info
ggplot(genei_df, aes(x=cav_grade, y=expression)) + geom_point()
