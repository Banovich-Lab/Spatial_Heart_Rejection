...
post_AMR_smpls <-  clustered_obj@meta.data %>% 
...
clustered_obj_linear_regression <- subset(clustered_obj, 
                                          subset = Sample %in% post_AMR_smpls)
...
for (ctn in 1:length(cts_to_analyze)) {
  ...
  # Extract the normalized data (data is log-transformed counts)
  ...
  # Summarize by Sample to calculate the mean for each gene and sample
  ...
  #        "/scratch/aoill/projects/heart_transplant/GLMM_results_CAV_AMR_post_treatment_all_rm_model_errors_fix_exp_20241126_in_progress_results.rds")
...
#        "/scratch/aoill/projects/heart_transplant/GLMM_results_CAV_AMR_post_treatment_all_rm_model_errors_fix_exp_20241126_FIX.rds")
...
write.csv(cr_post_cav_grade_all_CT_results_filter_merged, "/home/aoill/projects/heart_transplant/00_final/GLMM_CAV_grade_AMR_post_treatment_biopsies_NEW.csv",
          row.names = F, quote = F)