...
# Mixed pre-treatment - CAV score ----
## Subset Mixed pre-treatment samples ----
pre_Mixed_smpls <-  clustered_obj@meta.data %>% 
  filter(biopsy_rejection_type == "mixed_rejection") %>%
  filter(biopsy_timing == "pre") %>%
  dplyr::pull(Sample) %>% unique()


clustered_obj_linear_regression <- subset(clustered_obj, 
                                          subset = Sample %in% pre_Mixed_smpls)
...
mixed_pre_cav_grade_all_CT_results <- all_CT_results

saveRDS(mixed_pre_cav_grade_all_CT_results, 
        "/scratch/aoill/projects/heart_transplant/00_final/GLMM_CAV_grade_Mixed_pre_treatment_biopsies_revisions.rds")
saveRDS(all_CT_results_with_issues, 
        "/scratch/aoill/projects/heart_transplant/00_final/GLMM_CAV_grade_Mixed_pre_treatment_biopsies_revisions_with_issues.rds")


mixed_pre_cav_grade_all_CT_results <- readRDS("/scratch/aoill/projects/heart_transplant/00_final/GLMM_CAV_grade_Mixed_pre_treatment_biopsies_revisions.rds")

for (i in names(mixed_pre_cav_grade_all_CT_results)) {
  print(i)
  mixed_pre_cav_grade_all_CT_results[[i]]$p_value <- as.numeric(mixed_pre_cav_grade_all_CT_results[[i]]$p_value)
  mixed_pre_cav_grade_all_CT_results[[i]] <- mixed_pre_cav_grade_all_CT_results[[i]] %>%
    drop_na(p_value)
  mixed_pre_cav_grade_all_CT_results[[i]]$padj <- p.adjust(mixed_pre_cav_grade_all_CT_results[[i]]$p_value, method = "fdr")
}

saveRDS(mixed_pre_cav_grade_all_CT_results, 
        "/scratch/aoill/projects/heart_transplant/00_final/GGLMM_CAV_grade_Mixed_pre_treatment_biopsies_revisions_fixed.rds")
...
mixed_pre_cav_grade_all_CT_results_merged <- do.call(rbind, mixed_pre_cav_grade_all_CT_results)
unique(mixed_pre_cav_grade_all_CT_results_merged$ct)
table(mixed_pre_cav_grade_all_CT_results_merged$ct)

# Keep only significant genes
mixed_pre_cav_grade_all_CT_results_merged_sig <- mixed_pre_cav_grade_all_CT_results_merged %>%
  filter(padj <= 0.05)
mixed_pre_cav_grade_all_CT_results_merged_sig$n_samples_no0 <- as.numeric(mixed_pre_cav_grade_all_CT_results_merged_sig$n_samples_no0
)
nrow(mixed_pre_cav_grade_all_CT_results_merged_sig)
length(unique(mixed_pre_cav_grade_all_CT_results_merged_sig$genei))
length(unique(mixed_pre_cav_grade_all_CT_results_merged_sig$ct))
#   padj 0.05 - 55 significant genes across 14 cell types, 63 gene cell type pairs



# save merged as a csv 
# THIS IS SUPPLEMENTARY TABLE 8
write.csv(mixed_pre_cav_grade_all_CT_results_merged, "/home/aoill/projects/heart_transplant/00_final/revisions/GLMM_CAV_grade_Mixed_pre_treatment_biopsies_revisions_fixed.csv",
          row.names = F, quote = F)

write.csv(all_CT_results_with_issues_merged, "/home/aoill/projects/heart_transplant/00_final/revisions/GLMM_CAV_grade_Mixed_pre_treatment_biopsies_revisions_with_issues.csv",
          row.names = F, quote = F)


# Make a table with which gene cell type pairs had convergenc issues or small 
# variance issues
no_iss_results <- mixed_pre_cav_grade_all_CT_results_merged
...
glmms_with_issues_tbl$test <- "Mixed pre-treatment across CAV"
...
write.csv(glmms_with_issues_tbl, 
          "/home/aoill/projects/heart_transplant/00_final/revisions/GLMM_CAV_grade_Mixed_pre_treatment_biopsies_convergence_issues.csv",
          row.names = F, quote = F)
